...
class Sentinel():
  ...
  def shift_backwards(self, node, amount):
  ...
  def shift_forwards(self, node, amount):
...
nodes = []
for l in f:
  value = int(l.strip()) * decryption_key
  node = Node(value)
  nodes.append(node)
...
for round in range(10):
  print(f"Mixing round {round + 1}...")
  for i, node in enumerate(nodes):
    if node.val > 0:
      sentinel.shift_forwards(node, node.val % (len(nodes) - 1))
    else:
      sentinel.shift_backwards(node, abs(node.val) % (len(nodes) - 1))
