...
q = deque([((-1, 0), 0, False, False)])
visited = set()
while q:
  (i, j), steps, reached_end, reached_snacks = q.popleft()
  if (i, j) == (m - 1, n - 1):
    if reached_end and reached_snacks:
      # answer: 807
      print(steps + 1)
      break
    else:
      q.append(((m, n - 1), steps + 1, True, reached_snacks))
  if (i, j) == (0, 0) and reached_end:
    q.append(((-1, 0), steps + 1, reached_end, True))
  key = (i, j, steps, reached_end, reached_snacks)
  if key in visited:
    continue
  visited.add(key)
  ...
  for n_i, n_j in [(i + 1, j), (i - 1, j), (i, j + 1), (i, j - 1)]:
    if 0 <= n_i < m and 0 <= n_j < n and (n_i, n_j) not in nb:
      q.append(((n_i, n_j), steps + 1, reached_end, reached_snacks))
  if (i, j) not in nb:
    q.append(((i, j), steps + 1, reached_end, reached_snacks))
